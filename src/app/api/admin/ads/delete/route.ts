...
import { deleteCampaignFromForm } from "@/lib/ads/admin";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await deleteCampaignFromForm(formData, session.username);
    if (!result.ok) {
      return redirectWithStatus(request, "error", result.error ?? "Falha ao excluir campanha.");
    }

    revalidateAdPages();
    return redirectWithStatus(request, "ok", "Campanha removida com sucesso.");
  } catch (error) {
    const message = error instanceof Error ? error.message : "Erro inesperado ao excluir campanha.";
